...
def storage_action(**_args):
    ...
    # Get the target and an available slot within it. If the type is insert, the target will be the stash and the
    # source will be the inventory. Else, the source is the stash and the target are the inventory.
    target = Character.get_items(_args, _args['client']['character']['id'],
                                 'stash' if action_type == 'insert' else 'inventory', storage_number)
    available_slot = Character.get_available_inventory_slot(target)

    # If we do not have an available slot in our target, send a slot error. This means that there is no slot for the
    # item we wish to be moving.
    if available_slot is None:
        result.append_bytes([0x00, 0x44])  # No available slot
        return _args['socket'].sendall(result.packet)

    # Get source and check if the slot is present in it. The source will be the inventory if the type is insert,
    # else the source will be the storage/stash.
    source = Character.get_items(_args, _args['client']['character']['id'],
                                 'inventory' if action_type == 'insert' else 'stash', storage_number)
